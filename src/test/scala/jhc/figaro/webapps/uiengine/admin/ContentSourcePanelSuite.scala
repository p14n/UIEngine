...
class ContentSourcePanelSuite extends FunSuite {
  ...
  test("should display two feeders"){
    ...
    t.assertLabel("panel:feederSection:feeders:0:path","feeder1")
    t.assertLabel("panel:feederSection:feeders:1:path","feeder2")
  ...
  test("should delete a feeder"){
    val t = new WicketTester()
    t.startPanel(tps)
    t.clickLink("panel:feederSection:feeders:1:delete")
    assert(!map.contains("feeder2"))
  }
  test("should feed a feeder"){
    val t = new WicketTester()
    t.startPanel(tps)
    t.clickLink("panel:feederSection:feeders:0:feed")
